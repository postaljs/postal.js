	...
	describe("SubscriptionDefinition", function(){
		...
		describe("When debouncing the callback", function(){
			...
			it("should have only invoked debounced callback once", async(function() {
				...
				setTimeout(function() { sDefe.callback(2); },   20); // should not invoke callback
				setTimeout(function() { sDefe.callback(3); },   80); // should not invoke callback
				setTimeout(function() { sDefe.callback(4); },  250); // should not invoke callback
				setTimeout(function() { sDefe.callback(5); },  500); // should not invoke callback
				setTimeout(function() { sDefe.callback(6); }, 1000); // should invoke callback
				setTimeout(function() {
		...
		describe("When throttling the callback", function(){
			...
			it("should have only invoked throttled callback twice", async(function() {
				sDefe.callback(1); // starts the two second clock on debounce
				setTimeout(function() { sDefe.callback(800); }, 800); // should invoke callback
				for(var i = 0; i < 20; i++) {
					(function(x) { sDefe.callback(x); })(i);
				}
				setTimeout(function() {
