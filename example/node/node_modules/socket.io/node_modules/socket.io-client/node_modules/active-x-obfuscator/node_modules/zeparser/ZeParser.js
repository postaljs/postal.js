...
ZeParser.prototype = {
	...
	parse : function ( match ) {
		if ( match ) {
			...
			match = this.tokenizer.storeCurrentAndFetchNextToken( false, null, this.stack, true );
		...
		if ( cycled && match && match.name != 12/*eof*/ ) {
	...
	eatExpressions : function ( mayParseLabeledStatementInstead, match, stack, onlyOne, forHeader, isBreakOrContinueArg ) {
			...
			while ( parseAnotherExpression ) { // keep parsing lhs+operator as long as there is an operator after the lhs.
				...
				if ( match.value == '(' ) {
					...
					while ( foundAtLeastOneComma && match.value != ']' ) {
						...
						if ( !(/*is left hand side start?*/ match.name <= 6 || this.regexLhsStart.test( match.value )) && match.name != 14/*error*/ ) {
					...
					while ( match.value != '}' && match.name != 14/*error*/ ) { // will stop if next token is } or throw if not and no comma is found
						...
						if ( getset == 'get' ) {
							...
							if ( match.value == ':' ) {
								...
								match = this.tokenizer.storeCurrentAndFetchNextToken( false, match, stack );
								if ( match.value != '(' ) {
									match = this.failsafe( 'GetterSetterNameFollowedByOpenParen', match );
								}
								if ( this.ast ) { //#ifdef FULL_AST
									var lhp = match;
								} //#endif
							...
							if ( match.value == ':' ) {
								...
								if ( match.name != 2/*IDENTIFIER*/ ) {
									if ( match.value == ')' ) {
										...
										match = this.failsafe( 'IllegalSetterArgumentNameToken', match );
									}
								}
								match = this.tokenizer.storeCurrentAndFetchNextToken( false, match, stack );
								if ( match.value != ')' ) {
									if ( match.value == ',' ) {
										...
										match = this.failsafe( 'SetterHeaderShouldHaveClosingParen', match );
									}
								}
								if ( this.ast ) { //#ifdef FULL_AST
									match.twin = lhp;
						...
						if ( match.value == ',' ) {
							match = this.tokenizer.storeCurrentAndFetchNextToken( false, match, stack );
							if ( match.value == ',' ) {
								match = this.failsafe( 'IllegalDoubleCommaInObjectLiteral', match );
							}
						} else if ( match.value != '}' ) {
					...
					if (
						...
						if ( match.isAssignment ) {
							if ( !acceptAssignment ) {
								...
								this.failignore( 'AssignmentNotAllowedAfterNonAssignmentInExpression', match, stack );
							}
						}
						if ( isBreakOrContinueArg ) {
							match = this.failsafe( 'BreakOrContinueArgMustBeJustIdentifier', match );
						...
						if ( !match.isAssignment ) {
							parsedNonAssignmentOperator = true;
						} // last allowed assignment
						if ( this.ast ) { //#ifdef FULL_AST
						...
						if ( ternary ) {
							...
							if ( match.value != ':' ) {
								if ( match.value == ',' ) {
									match = this.failsafe( 'TernarySecondExpressionCanNotContainComma', match );
								}
								else {
									match = this.failsafe( 'UnfinishedTernaryOperator', match );
								}
							}
							if ( this.ast ) { //#ifdef FULL_AST
								// we build a stack to ensure any whitespace doesnt break the 1+(n*2) children rule for expressions
		...
		if ( this.ast ) { //#ifdef FULL_AST
			...
			if ( pstack[0] ) {
				pstack[0].numberOfExpressions = parsedExpressions;
			}
	...
	eatFunctionParametersAndBody : function ( match, stack, div, funcToken ) {
		...
		if ( match.name == 2/*IDENTIFIER*/ ) { // params
			...
			if ( this.ast ) { //#ifdef FULL_AST
				if ( !funcToken.paramNames ) {
					funcToken.paramNames = [];
				}
		...
		if ( match.value != ')' ) {
			match = this.failsafe( 'ExpectedFunctionHeaderClose', match );
		} // TOFIX: can be various things here...
		match = this.tokenizer.storeCurrentAndFetchNextToken( false, match, stack );
	...
	eatVarDecl : function ( match, stack, forHeader ) {
		...
		do {
			...
			if ( match.name == 12/*eof*/ ) {
				if ( first ) {
					match = this.failsafe( 'VarKeywordMissingName', match );
				}
				// else, ignore. TOFIX: return?
				else {
					match = this.failsafe( 'IllegalTrailingComma', match );
				}
			...
			if ( match.value == '=' ) {
				...
				if ( !(/*is left hand side start?*/ match.name <= 6 || match.name == 14/*error*/ || this.regexLhsStart.test( match.value )) ) {
	...
	eatContinue : function ( match, stack ) {
		...
		if ( !match.newline && match.value != ';' && match.name != 12/*EOF*/ && match.value != '}' ) {
			...
			if ( match.value != ';' && !match.newline && match.name != 12/*eof*/ && match.value != '}' ) {
				match = this.failsafe( 'BreakOrContinueArgMustBeJustIdentifier', match );
			}
		}
		match = this.eatSemiColon( match, stack );
	...
	eatThrow : function ( match, stack ) {
		...
		match = this.tokenizer.storeCurrentAndFetchNextToken( false, match, stack ); // may not have line terminator...
		if ( match.newline ) {
			match = this.failsafe( 'ThrowCannotHaveReturn', match );
		}
		if ( match.value == ';' ) {
	...
	eatSwitchBody : function ( match, stack ) {
		...
		if ( lastMatch == match ) {
			this.failsafe( 'UnexpectedInputSwitch', match );
		}
	...
	eatCatch : function ( match, stack ) {
		...
		if ( this.ast ) { //#ifdef FULL_AST
			...
			while ( nonCatchScope.catchScope ) {
				nonCatchScope = nonCatchScope[0];
			}

			// get catch id, which is governed by the function/global scope only
			if ( !nonCatchScope.catches ) {
				nonCatchScope.catches = [];
			}
			...
			if ( catchScopeBackup.global ) {
				this.scope.push( catchScopeBackup[0] );
				...
				this.scope.push( catchScopeBackup[1], catchScopeBackup[2] );
			} // function scope, copy this and arguments
		} //#endif
	...
	eatBlock : function ( match, stack ) {
		...
		if ( match.value != '}' ) {
			match = this.failsafe( 'BlockCurlyClose', match );
		}
		if ( this.ast ) { //#ifdef FULL_AST
			match.twin = lhc;
			lhc.twin = match;
