...
<Aside type="note">
    `connectToIframe` / `connectToWorker` throw `PostalHandshakeTimeoutError` if the other side
    doesn't complete the handshake within the timeout (default 5 seconds). Make sure the iframe or
    worker calls its corresponding connect function.
</Aside>
