...
postal supports compile-time payload inference via module augmentation:

```ts
import { getChannel } from "postal";

// Define your channel's topic → payload map
type OrderTopics = {
    "item.placed": { sku: string; qty: number };
    "item.cancelled": { sku: string; reason: string };
};

// Pass the map as a generic — payload types are now enforced
const orders = getChannel<"orders">("orders");
```

For project-wide type inference without generics at every call site, use module augmentation:
...
// Now getChannel("orders") infers the full topic map automatically
const orders = getChannel("orders");
