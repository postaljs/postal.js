...
- `postal-transport-messageport` — iframes, Web Workers, and Node.js worker threads
...
6. **Replace `envelope.timeStamp`** → `envelope.timestamp` (lowercase `s`) — it's now `Date.now()` (a number), not a `Date` object
7. **Replace subscription chaining** — see [Removed subscription methods](#removed-subscription-methods)
8. **Replace `.unsubscribe()`** with the returned function
9. **Update wire taps** — `addWireTap` → `addWiretap`, update callback signature
10. **Migrate federation** to [transports](/guide/transports/) if applicable
11. **Replace `postal.reset()`** → `resetChannels()` in tests
12. **Run your tests** — most remaining issues will be `envelope.data` → `envelope.payload` misses
</Steps>
