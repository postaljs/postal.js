 ...
 *   3. Transport setup — BroadcastChannel bridges across same-origin tabs
 *   4. Channel — getChannel("sync") scopes all messages
 *   5. Subscriptions — auth.login, auth.logout, theme.changed, prefs.changed
 *      Each subscriber updates state, persists, and re-renders. Single code
 *      path for both local publishes and remote messages from other tabs.
